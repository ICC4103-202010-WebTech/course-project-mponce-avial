class Admin::BlacklistsController < AdminController
  ...
  def index
    ...
    @reports_users = Blacklist.where(:reported_type => "user")
    @reports_events = Blacklist.where(:reported_type => "event")
    @reports_organizations = Blacklist.where(:reported_type => "organization")
  end
  ...
  def destroy
    @blacklist.destroy
    redirect_to admin_blacklists_path,notice: "The report - " + @blacklist.message + " - was deleted"
  end
